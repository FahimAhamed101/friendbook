...
import com.app.myfriend.group.CommentGroupActivity
import com.app.myfriend.group.CreateGroupPostActivity
import com.app.myfriend.pages.CommentPagesActivity
import com.app.myfriend.pages.CreatePostPagesActivity
...
class StickersPost : AppCompatActivity(), StipopDelegate {
    ...
    override fun onStickerSelected(sticker: SPSticker): Boolean {
        ...
        if (intent.getStringExtra("activity").equals("post")){
            ...
            if (intent.getStringExtra("activity").equals("pages")){
                val i = Intent(this, CreatePostPagesActivity::class.java)
                i.putExtra("gif",  sticker.stickerImg.toString())
                startActivity(i)
                finish()
            }else
        if (intent.getStringExtra("activity").equals("group")){
            ...
            if (intent.getStringExtra("activity").equals("comment")){
                ...
                if (intent.getStringExtra("activity").equals("groupcomment")){
                    ...
                    startActivity(i)
                    finish()
                }else if(intent.getStringExtra("activity").equals("pagecomment")){
                    val i = Intent(this, CommentPagesActivity::class.java)
                    i.putExtra("gif",  sticker.stickerImg.toString())
                    i.putExtra("postID", intent.getStringExtra("postID"))
                    i.putExtra("id", intent.getStringExtra("groupId"))
                    startActivity(i)
                    finish()
                }
