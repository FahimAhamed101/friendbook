...
import com.app.myfriend.GetTimeAgo;
import com.app.myfriend.MediaViewActivity;
import com.app.myfriend.R;
import com.app.myfriend.model.ModelPost;
import com.app.myfriend.model.ModelUser;
import com.app.myfriend.notifications.Data;
import com.app.myfriend.notifications.Sender;
import com.app.myfriend.notifications.Token;
import com.app.myfriend.post.CommentActivity;
import com.app.myfriend.profile.UserProfileActivity;
import com.app.myfriend.search.SearchActivity;
import com.app.myfriend.send.SendToGroupActivity;
import com.app.myfriend.send.SendToUserActivity;
import com.app.myfriend.watchParty.StartPartyActivity;
import com.app.myfriend.watchParty.StartYouTubeActivity;
import com.app.myfriend.who.LikedActivity;
...
public class AdapterPost extends RecyclerView.Adapter<AdapterPost.MyHolder>{
    ...
    private void displayAd(MyHolder holder) {

        FirebaseDatabase.getInstance().getReference("AdsPost").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()){
                    ...
                    int i=0;
                    String themeTune = null;
                    for (DataSnapshot snap : snapshot.getChildren()) {
                        if(i == randomNumber) {
                            themeTune = snap.getKey();
                            break;
                        }
                        i++;

                    }
                    showAd(themeTune, holder);
                }
            }
    ...
    private void showAd(String themeTune, MyHolder holder) {

        FirebaseDatabase.getInstance().getReference("AdsPost").child(themeTune).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {

                if (!snapshot.child("expiry").getValue().toString().equals("no")){
                    holder.ad_ly.setVisibility(View.VISIBLE);
                    ...
                    FirebaseDatabase.getInstance().getReference().child("Users").child(snapshot.child("id").getValue().toString()).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot napshot) {
                            if (napshot.exists()){
                                holder.ads_tv_name.setText(napshot.child("name").getValue().toString());

                                holder.ads_tv_username.setText(napshot.child("username").getValue().toString());
                                if (!napshot.child("photo").getValue().toString().isEmpty()){
                                    Picasso.get().load(napshot.child("photo").getValue().toString()).into(holder.ads_profile_photo);
                                }else {
                                    Picasso.get().load(R.drawable.avatar).into(holder.ads_profile_photo);
                                }
                            }
                        }
                    ...
                    holder.ads_tv_tweet_text.setLinkText(snapshot.child("text").getValue().toString());
                    holder.ads_tv_tweet_text.setOnLinkClickListener(new SocialTextView.OnLinkClickListener() {
                        ...
                        public void onLinkClicked(int i, String s) {
                            ...
                            int views = Integer.parseInt(snapshot.child("pViews").getValue().toString())+1;
                            HashMap<String, Object> hashMap = new HashMap<>();
                            hashMap.put("pViews", String.valueOf(views));
                            FirebaseDatabase.getInstance().getReference("AdsPost").child(themeTune).updateChildren(hashMap);
                    ...
                    if (snapshot.child("type").getValue().toString().equals("image")){
                        ...
                        Picasso.get().load(snapshot.child("image").getValue().toString()).into(holder.ads_media);
                        holder.ads_play.setVisibility(View.GONE);

                    }else if (snapshot.child("type").getValue().toString().equals("video")){
                        holder.ads_media_layout.setVisibility(View.VISIBLE);
                        holder.ads_media.setVisibility(View.VISIBLE);
                        Glide.with(context).asBitmap().load(snapshot.child("video").getValue().toString()).thumbnail(0.1f).into(holder.ads_media);
                    ...
                    holder.ads_media_layout.setOnClickListener(v -> {
                        ...
                        if (snapshot.child("type").getValue().toString().equals("image")){
                            ...
                            intent.putExtra("uri", snapshot.child("image").getValue().toString());
                            context.startActivity(intent);
                        }else
                        if (snapshot.child("type").getValue().toString().equals("video")){
                            Intent intent = new Intent(context, MediaViewActivity.class);
                            intent.putExtra("type", snapshot.child("type").getValue().toString());
                            intent.putExtra("uri", snapshot.child("video").getValue().toString());
                            context.startActivity(intent);
                        }
    ...
    static class MyHolder extends RecyclerView.ViewHolder{
        ...
        public MyHolder(@NonNull View itemView) {
            ...
            ad_ly = itemView.findViewById(R.id.ad_ly);
                    ...
                    try {
                        ...
                        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest("https://fcm.googleapis.com/fcm/send", jsonObject, response -> Timber.d("onResponse%s", response.toString()), error -> Timber.d("onResponse%s", error.toString())){
                            ...
                            public Map<String, String> getHeaders() {
                                ...
                                headers.put("Authorization", "key=AAAA1HpIboU:APA91bGTF-wbYL6vkAbUlkjiGMBLGISQjAMTMJsuEalHgfqVZsba5mHxUHP3dGQ1W63zpLFZbWn-tRjc8_v252ykJKlB3XpT2cqM-7duD2L89uKbyUthhC6wTHUEqms6gFSZlYGlbqUk");
                                return headers;
                            }
                        };
